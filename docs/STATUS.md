...
- 再进入一次自定义选人阶段，运行 `leagueakari-probe`，确认默认摘要能显示双方公开 pick / ban 的英雄名称。
- 确认 `composition analysis` 的解释口吻是否符合“帮你理解这一局应该怎么赢”。
- 决定下一步先做 WebSocket 实时监听，还是先搭 Tauri UI。

## 推荐下一步

优先做 WebSocket 实时监听。

原因：UI 可以晚一点，但实时监听能验证选人变化能否稳定推送，这是后续阵容分析和 Tauri 前端的核心数据流。
